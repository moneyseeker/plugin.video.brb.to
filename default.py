...
__settings__ = xbmcaddon.Addon(id='plugin.video.brb.to')
...
cookiepath = os.path.join(__addondir__, 'cookies.pickle')
...
def save_cookies_lwp(cookiejar, filename):
    ...
    open(cookiepath, 'w').write(dumps(dic))
...
def GET(url, referer, post_params = None):
    ...
    url = getFullUrl(url)
    #Если нужно залогиниться, нужно запостить
    if post_params != None:
        req = requests.post(url, data=post_params, headers=headers, allow_redirects=False)
        save_cookies_lwp(req.cookies, cookiepath)
        req = requests.get(getFullUrl('/'), headers=headers, cookies=load_cookies_from_lwp(cookiepath))
    #А если логиниться не нужно, постить нечего
    else:
...
def check_login():
    ...
    if len(login) > 0:
        ...
        if userPanel == "":
            ...
            if "b-header__user-panel" in loginResponse:
                userPanel = "found"
...
def render_search_results(params):
    ...
    if len(items) == 0:
        ...
        if nextPageLink != None:
            ...
            li.setProperty('IsPlayable', 'false')
            uri = construct_request({
                    'href': httpSiteUrl + str(nextPageLink['href']),
